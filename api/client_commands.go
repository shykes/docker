...
func (cli *DockerCli) CmdHelp(job *engine.Job) engine.Status {
	if nArgs := len(job.Args); nArgs > 1 {
		return job.Errorf("Usage: %s COMMAND", job.Name)
	} else if nArgs == 1 {
		// FIXME: create sub-job with job.Job()
		helpJob := job.Eng.Job(job.Args[0], "--help")
		helpJob.Stdout = job.Stdout
		helpJob.Stderr = job.Stderr
		job.Run()
		return job.Status()
	} else if nArgs == 0 {
...
func (cli *DockerCli) CmdInsert(job *engine.Job) engine.Status {
	cmd := cli.Subcmd("insert", "IMAGE URL PATH", "Insert a file from URL in the IMAGE at PATH")
	if err := cmd.Parse(job.Args); err != nil {
		return engine.StatusOK
	}
	if cmd.NArg() != 3 {
		cmd.Usage()
	...
	err := cli.stream("POST", "/images/"+cmd.Arg(0)+"/insert?"+v.Encode(), nil, cli.out, nil)
	if err != nil {
...
func (cli *DockerCli) CmdBuild(job *engine.Job) engine.Status {
	...
	if cmd.Arg(0) == "-" {
		...
		if _, err = os.Stat(filename); os.IsNotExist(err) {
			return job.Errorf("no Dockerfile found in %s", cmd.Arg(0))
		}
	...
	if jerr, ok := err.(*utils.JSONError); ok {
		...
		job.Errorf("%s\n", jerr.Message)
		return engine.Status(jerr.Code)
	}
	return job.Error(err)
}

// 'docker login': login / register a user to registry service.
func (cli *DockerCli) CmdLogin(job *engine.Job) engine.Status {
	...
	if username != authconfig.Username {
		if password == "" {
			...
			if password == "" {
				return job.Errorf("Error : Password Required")
			}
		}
...
func (cli *DockerCli) CmdWait(job *engine.Job) engine.Status {
	...
	if encounteredError != nil {
		return job.Error(encounteredError)
	}
	return engine.StatusOK
}

// 'docker version': show version information
func (cli *DockerCli) CmdVersion(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdInfo(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdStop(job *engine.Job) engine.Status {
	...
	if err := encounteredError; err != nil {
return job.Error(err)
}
return engine.StatusOK
}

func (cli *DockerCli) CmdRestart(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdStart(job *engine.Job) engine.Status {
	...
	if *attach || *openStdin {
		if cmd.NArg() > 1 {
			return job.Errorf("Impossible to start and attach multiple containers at once.")
		}
	...
	if *openStdin || *attach {
		...
		return job.Error(<-cErr)
	}
	return engine.StatusOK
}

func (cli *DockerCli) CmdInspect(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdTop(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdPort(job *engine.Job) engine.Status {
	...
	if frontends, exists := out.NetworkSettings.Ports[nat.Port(port+"/"+proto)]; exists && frontends != nil {
		...
		return job.Errorf("Error: No public port '%s' published for %s", cmd.Arg(1), cmd.Arg(0))
	}
	return engine.StatusOK
}

// 'docker rmi IMAGE' removes all images with the name IMAGE
func (cli *DockerCli) CmdRmi(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdHistory(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdRm(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdKill(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdImport(job *engine.Job) engine.Status {
	...
	err := cli.stream("POST", "/images/create?"+v.Encode(), in, cli.out, nil)
	if err != nil {
		return job.Error(err)
	}
	return engine.StatusOK
...
func (cli *DockerCli) CmdPush(job *engine.Job) engine.Status {
	...
	if len(strings.SplitN(name, "/", 2)) == 1 {
		...
		return job.Errorf("Impossible to push a \"root\" repository. Please rename your repository in <user>/<repo> (ex: %s/%s)", username, name)
	}
	...
	push := func(authConfig auth.AuthConfig) error {
		...
		err = cli.stream("POST", "/images/"+name+"/push?"+v.Encode(), nil, cli.out, map[string][]string{ "X-Registry-Auth": registryAuthHeader})
		if err != nil {
			return err
		}
		return nil
	...
	if err := push(authConfig); err != nil {
		if strings.Contains(err.Error(), "Status 401") {
			...
			if err := job.Eng.Job("login", "endpoint").Run(); err != nil {
				return job.Errorf("login: %s", err)
			}
			authConfig := cli.configFile.ResolveAuthConfig(endpoint)
			if err := push(authConfig); err != nil {
				return job.Errorf("push: %s", err)
			}
			return engine.StatusOK
		}
		return job.Error(err)
...
func (cli *DockerCli) CmdPull(job *engine.Job) engine.Status {
	...
	pull := func(authConfig auth.AuthConfig) error {
		...
		err = cli.stream("POST", "/images/create?"+v.Encode(), nil, cli.out, map[string][]string{ "X-Registry-Auth": registryAuthHeader})
		if err != nil {
			return err
		}
		return nil
...
func (cli *DockerCli) CmdImages(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdPs(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdCommit(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdEvents(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdExport(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdDiff(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdLogs(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdAttach(job *engine.Job) engine.Status {
	...
	if !container.State.Running {
		return job.Errorf("Impossible to attach to a stopped container, start it first")
	}
...
func (cli *DockerCli) CmdSearch(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdTag(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdRun(job *engine.Job) engine.Status {
	// FIXME: just use runconfig.Parse already
	config, hostConfig, cmd, err := runconfig.ParseSubcommand(cli.Subcmd("run", "[OPTIONS] IMAGE [COMMAND] [ARG...]", "Run a command in a new container"), job.Args, nil)
	if err != nil {
		return job.Error(err)
	}
	if config.Image == "" {
	...
	if len(hostConfig.ContainerIDFile) > 0 {
		if _, err := os.Stat(hostConfig.ContainerIDFile); err == nil {
			return job.Errorf("cid file found, make sure the other container isn't running or delete %s", hostConfig.ContainerIDFile)
		}
		if containerIDFile, err = os.Create(hostConfig.ContainerIDFile); err != nil {
			return job.Errorf("failed to create the container ID file: %s", err)
		}
	...
	if len(hostConfig.ContainerIDFile) > 0 {
		if _, err = containerIDFile.Write([]byte(runResult.Get("Id"))); err != nil {
			return job.Errorf("failed to write the container ID to the file: %s", err)
		}
	}
...
func (cli *DockerCli) CmdCp(job *engine.Job) engine.Status {
	...
	if len(info) != 2 {
		return job.Errorf("Error: Path not specified")
	}
...
func (cli *DockerCli) CmdSave(job *engine.Job) engine.Status {
...
func (cli *DockerCli) CmdLoad(job *engine.Job) engine.Status {
