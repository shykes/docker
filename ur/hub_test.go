...
func TestHubSendEmpty(t *testing.T) {
	...
	if r, w, err := hub.Send(&beam.Message{"hello", nil, nil}, beam.R|beam.W); err != nil {
		t.Fatal(err)
	} else if r != nil {
		t.Fatalf("%#v", r)
	} else if w != nil {
...
func TestHubSendOneHandler(t *testing.T) {
	...
	testutils.Timeout(t, func() {
		in, _, err := hub.Send(&beam.Message{"register", nil, nil}, beam.R)
		if err != nil {
			t.Fatal(err)
		}
		go func() {
			if _, _, err := hub.Send(&beam.Message{"hello", nil, nil}, 0); err != nil {
				t.Fatal(err)
			}
		}()
